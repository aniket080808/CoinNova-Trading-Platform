...
import { transactions, tradeJournals, achievements, behaviorHistoryDaily } from "../db/schema.js";
...
import { analyzeTradeReplay } from "../services/behavior/ReplayEngine.js";
import type { RawTransaction, BehaviorAnalysisResult } from "../services/behavior/types.js";

const router = Router();

// ── Helper ─────────────────────────────────────────────────────────────────
async function fetchUserTxs(userId: string): Promise<RawTransaction[]> {
...
router.post("/scan", requireAuth, async (req, res) => {
  try {
    ...
    let grade = "C";
    if (scoreData.behaviorScore >= 90) grade = "A+";
    else if (scoreData.behaviorScore >= 80) grade = "A";
    else if (scoreData.behaviorScore >= 70) grade = "B";
    else if (scoreData.behaviorScore >= 60) grade = "C+";
    else if (scoreData.behaviorScore < 40) grade = "F";
