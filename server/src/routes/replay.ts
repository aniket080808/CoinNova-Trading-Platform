...
router.get("/history", requireAuth, async (req, res) => {
  try {
    ...
    let reviewsMap: Record<string, typeof replayReviews.$inferSelect> = {};
    ...
    if (replayIds.length > 0) {
      ...
      for (const rid of replayIds) {
        const [review] = await db
          ...
          .limit(1);
        if (review) reviewsMap[rid] = review;
      }
    }
...
router.get("/:id/trades", requireAuth, async (req, res) => {
  try {
    ...
    const [replay] = await db
      ...
      .from(tradingReplays)
      .where(eq(tradingReplays.id, id as string))
      .limit(1);
    ...
    const trades = await db
      ...
      .where(eq(replayTransactions.replayId, id as string));
...
router.post("/save", requireAuth, validate(saveReplaySchema), async (req, res) => {
  try {
    const {
      coinId, symbol, difficulty,
      startCapital, finalBalance, userReturn, buyAndHoldReturn,
      totalTrades, winRate, maxDrawdown, averageHoldingTime, replayDuration,
      trades, review,
    } = req.body;
    const userId = req.user!.userId;
    ...
    if (trades && trades.length > 0) {
      await db.insert(replayTransactions).values(
        trades.map((t: any) => ({
