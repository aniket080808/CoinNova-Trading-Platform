...
router.post("/pin/request-otp", requireAuth, async (req, res) => {
  try {
    ...
    if (action === "setup") {
      await sendPinSetupEmail(email, otp);
    } else {
...
router.post("/pin/change", requireAuth, validate(changePinSchema), async (req, res) => {
  try {
    ...
    if (!user || !user.transactionPin) return res.status(400).json({ error: "PIN not set" });

    const isValid = await bcrypt.compare(oldPin, user.transactionPin);
    if (!isValid) return res.status(403).json({ error: "Invalid old PIN" });
    ...
    if (!otp || new Date() > otp.expiresAt) return res.status(400).json({ error: "Invalid or expired OTP" });
...
router.post("/email/request-otp", requireAuth, async (req, res) => {
  try {
    ...
    if (role === "admin") return res.status(403).json({ error: "Admin email is static" });
...
const settingsHandler = async (req: any, res: any) => {
  try {
    ...
    if (currencyPreference) updates.currencyPreference = currencyPreference;
