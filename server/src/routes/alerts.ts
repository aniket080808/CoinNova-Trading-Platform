...
router.get("/", requireAuth, async (req: Request, res: Response) => {
  ...
  } catch (err) { console.error("Alerts list error:", err); res.status(500).json({ error: "Internal server error" }); }
});
...
router.post("/", requireAuth, validate(createSchema), async (req: Request, res: Response) => {
  ...
  } catch (err) { console.error("Alert create error:", err); res.status(500).json({ error: "Internal server error" }); }
});
