...
import type { GuardianResult, RawTransaction, MistakeResult, GuardianWarning } from "./types.js";
...
export function validateTrade(
  ...
  // 3. Overtrading check
  const last24hTrades = history.filter(
    t => (new Date().getTime() - t.createdAt.getTime()) < 24 * 60 * 60 * 1000
  );
  ...
  const similarTrades = history.filter(
    t => t.type === "buy" && (t.confidence ?? 50) >= proposed.confidence - 10
  );
