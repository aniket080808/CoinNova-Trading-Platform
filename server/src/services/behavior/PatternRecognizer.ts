...
import type {
  RawTransaction, PatternResult, WinRateByReason, WinRateByConfidence, HourlyActivity
} from "./types.js";
...
function pairTrades(coinTxs: RawTransaction[]): Array<{
  buy: RawTransaction; sell: RawTransaction; holdHours: number; pnlPct: number;
}> {
...
export function recognizePatterns(txs: RawTransaction[]): PatternResult {
  ...
  // ── Holding period + PnL ─────────────────────────────────────────────────
  const allPairs: Array<{ buy: RawTransaction; sell: RawTransaction; holdHours: number; pnlPct: number }> = [];
  for (const coinTxs of Object.values(groups)) allPairs.push(...pairTrades(coinTxs));
  ...
  const coinTradeCounts: Record<string, { symbol: string; count: number; wins: number; total: number }> = {};
  for (const pairs of Object.values(groups)) {
    ...
    if (!coinTradeCounts[coinId]) coinTradeCounts[coinId] = { symbol, count: 0, wins: 0, total: 0 };
  ...
  const byWinRate = withPairs.sort((a, b) =>
  ...
  // ── Win rate by reason ───────────────────────────────────────────────────
  const reasonGroups: Record<string, { wins: number; total: number }> = {};
  for (const p of allPairs) {
    const reason = p.buy.reason ?? "unspecified";
    if (!reasonGroups[reason]) reasonGroups[reason] = { wins: 0, total: 0 };
    reasonGroups[reason].total++;
    if (p.pnlPct > 0) reasonGroups[reason].wins++;
  }
  const winRateByReason: WinRateByReason[] = Object.entries(reasonGroups).map(([reason, v]) => ({
    reason,
    trades: v.total,
    wins: v.wins,
  ...
  const winRateByConfidence: WinRateByConfidence[] = CONFIDENCE_BUCKETS.map(bucket => {
  ...
  for (const tx of buys) hourCounts[tx.createdAt.getHours()]++;
  const hourlyActivity: HourlyActivity[] = hourCounts.map((trades, hour) => ({ hour, trades }));
