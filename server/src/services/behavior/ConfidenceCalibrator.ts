...
import type { RawTransaction, CalibrationResult, CalibrationBucket } from "./types.js";
...
function pairTrades(txs: RawTransaction[]): Array<{ conf: number; win: boolean }> {
  ...
  const pairs: Array<{ conf: number; win: boolean }> = [];
  for (const coinTxs of Object.values(groups)) {
    ...
    for (const tx of sorted) {
      if (tx.type === "buy") buys.push(tx);
        ...
        if (buy.confidence == null) continue;
...
export function calibrateConfidence(txs: RawTransaction[]): CalibrationResult {
