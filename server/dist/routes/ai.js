...
router.post("/suggest", requireAuth, validate(suggestSchema), async (req, res) => {
    try {
        ...
        const holdingsSummary = holdings.map((h) => `${h.symbol.toUpperCase()}: ${h.amount.toFixed(4)} coins, avg $${h.avgPrice.toFixed(2)}, now $${h.currentPrice.toFixed(2)}`).join("\n");
...
router.post("/replay-review", requireAuth, validate(replayReviewSchema), async (req, res) => {
    try {
        ...
        const tradeSummary = trades
            ? `The user made ${trades.length} total trades. Sample actions: ${trades.slice(0, 5).map((t) => `${t.action} ${t.quantity.toFixed(4)} @ $${t.price.toFixed(2)}`).join(", ")}.`
...
router.post("/journal-analysis", requireAuth, async (req, res) => {
    try {
        ...
        const panicScore = Math.max(0, 100 - Math.floor((journals.filter((j) => j.sentiment === "panic").length) * 15));
        const disciplineScore = Math.min(100, 20 + Math.floor((journals.filter((j) => j.sentiment === "calm").length) * 20));
...
router.post("/portfolio-health", requireAuth, validate(portfolioHealthSchema), async (req, res) => {
    try {
        ...
        const totalHoldingsValue = holdings.reduce((s, h) => s + h.currentPrice * h.amount, 0);
        ...
        const stablecoinValue = holdings
            .filter((h) => STABLECOINS.has(h.symbol.toLowerCase()) || STABLECOINS.has(h.coinId.toLowerCase()))
            .reduce((s, h) => s + h.currentPrice * h.amount, 0);
        ...
        const bluechipValue = holdings
            .filter((h) => BLUECHIPS.has(h.symbol.toLowerCase()) || BLUECHIPS.has(h.coinId.toLowerCase()))
            .reduce((s, h) => s + h.currentPrice * h.amount, 0);
        ...
        const hhi = weightedHoldings.reduce((s, h) => s + Math.pow(h.weight * 100, 2), 0);
        ...
        if (stableRatio >= ph.IDEAL_STABLE_RATIO_MIN && stableRatio <= ph.IDEAL_STABLE_RATIO_MAX)
            stabilityScore = 90;
        else if (stableRatio < 0.05)
            stabilityScore = 20;
        else if (stableRatio < ph.IDEAL_STABLE_RATIO_MIN)
            stabilityScore = 50 + ((stableRatio - 0.05) / (ph.IDEAL_STABLE_RATIO_MIN - 0.05)) * 40;
        else if (stableRatio > ph.IDEAL_STABLE_RATIO_MAX && stableRatio <= 0.60)
            stabilityScore = 90 - ((stableRatio - ph.IDEAL_STABLE_RATIO_MAX) / (0.60 - ph.IDEAL_STABLE_RATIO_MAX)) * 40;
        else
            stabilityScore = 15;
        ...
        const concentrationScore = Math.round(Math.max(0, Math.min(100, concentrationRisk <= ph.MAX_CONCENTRATION ? 100 : 100 - ((concentrationRisk - ph.MAX_CONCENTRATION) / (1 - ph.MAX_CONCENTRATION)) * 100)));
        // Overall health score (weighted average)
        const overallScore = Math.round(diversificationScore * 0.30 +
            stabilityScore * 0.25 +
            qualityScore * 0.20 +
