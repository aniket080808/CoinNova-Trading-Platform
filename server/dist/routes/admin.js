...
router.get("/stats", requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const [volume] = await db.select({ total: sql `coalesce(sum(${transactions.total}::numeric), 0)` }).from(transactions);
        res.json({
...
router.get("/transactions", requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        console.error("Admin txs error:", err);
        res.status(500).json({ error: "Internal server error" });
    }
});
...
router.post("/withdrawals/:id/reject", requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        await db.transaction(async (trx) => {
            ...
            await trx.update(transactions).set({ status: "failed" }).where(eq(transactions.id, id));
        });
