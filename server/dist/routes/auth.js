...
import { users, wallets, otpCodes } from "../db/schema.js";
...
router.post("/verify-otp", validate(verifyOtpSchema), async (req, res) => {
    try {
        ...
        const [otp] = await db
            .select()
            .from(otpCodes)
            .where(and(eq(otpCodes.userId, user.id), eq(otpCodes.code, code), eq(otpCodes.type, type), eq(otpCodes.used, false)))
        ...
        const [fullUser] = await db
            ...
            hasPin: sql `${users.transactionPin} IS NOT NULL`,
...
router.post("/reset", validate(resetSchema), async (req, res) => {
    try {
        ...
        const [otp] = await db
            .select()
            .from(otpCodes)
            .where(and(eq(otpCodes.userId, user.id), eq(otpCodes.code, code), eq(otpCodes.type, "password_reset"), eq(otpCodes.used, false)))
            .limit(1);
        if (!otp || new Date() > otp.expiresAt) {
            res.status(400).json({ error: "Invalid or expired code" });
            return;
