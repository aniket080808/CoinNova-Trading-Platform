...
router.post("/deposit/confirm", requireAuth, async (req, res) => {
    try {
        ...
        await db.transaction(async (tx) => {
            const [existingTx] = await tx.select().from(transactions).where(eq(transactions.stripeSessionId, sessionId)).limit(1);
            ...
            if (existingTx) {
                ...
                await tx.insert(transactions).values({ userId, type: "deposit", amount: String(amountUsd), total: String(amountUsd), status: "completed", toDest: "Stripe", stripeSessionId: sessionId });
            }
        ...
        if (userId && amountUsd > 0) {
            try {
                await db.transaction(async (tx) => {
                    ...
                    if (user)
                        await sendDepositConfirmation(user.email, amountUsd);
                });
            }
            catch (err) {
