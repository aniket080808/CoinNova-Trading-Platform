...
// ─────────────────────────────────────────────────────────────────────────────
function calculateDiscipline(mistakes) {
...
function calculatePatience(mistakes, patterns) {
...
function calculateConfidence(patterns) {
    ...
    if (!highConfBucket || highConfBucket.trades === 0)
        return 50; // Neutral if no data
    return Math.max(0, Math.min(100, highConfBucket.winRate));
}
function calculateRiskControl(mistakes, patterns) {
...
export function generateTradingDNA(txs, mistakes, patterns) {
    ...
    const behaviorScore = Math.round((discipline + patience + riskControl + consistency + emotionalControl) / 5);
