...
export function detectMistakes(txs) {
    ...
    // ── 2. Panic Sell ───────────────────────────────────────────────────────
    const panicSells = tradeTxs.filter(t => t.type === "sell" && t.reason !== "profit_booking" && t.reason !== "long_term" && (t.confidence ?? 0) > 70);
    ...
    for (const pairs of Object.values(groups)) {
        for (const p of pairTrades(pairs)) {
            if (p.holdHours < QUICK_EXIT_HOURS && p.pnlPct > 0)
                earlyExits++;
        }
    }
    if (earlyExits > 0) {
    ...
    for (const pairs of Object.values(groups)) {
        for (const p of pairTrades(pairs)) {
            if (p.holdHours > HOLD_LOSS_HOURS && p.pnlPct < 0)
                longHoldLosers++;
        }
    }
    if (longHoldLosers > 0) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        const window = sorted.filter(t => Math.abs(t.createdAt.getTime() - sorted[i].createdAt.getTime()) < 24 * 60 * 60 * 1000);
        if (window.length > 3) {
            overtradingEvents++;
            i += window.length - 1;
        }
    ...
    for (const pairs of Object.values(groups)) {
        for (const p of pairTrades(pairs)) {
            if ((p.buy.confidence ?? 0) >= 80 && p.pnlPct < -5)
                highConfLosses++;
        }
    }
    if (highConfLosses > 0) {
    ...
    return results.sort((a, b) => severityOrder[a.severity] - severityOrder[b.severity] || b.frequency - a.frequency);
}
export function findMostExpensiveHabit(txs, mistakes) {
    ...
    for (const p of allPairs) {
        ...
        if (p.buy.reason === "fomo")
            category = "FOMO Buying";
        else if (p.sell.reason === "panic")
            category = "Panic Selling";
        else if (p.holdHours > HOLD_LOSS_HOURS)
            category = "Holding Losers Too Long";
        else if ((p.buy.confidence ?? 0) >= 80)
            category = "Overconfident Buying";
        if (category !== "Unknown") {
