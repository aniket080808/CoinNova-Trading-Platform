...
interface DemoState {
  ...
  buy: (coin: { id: string; symbol: string; name: string; image: string }, usd: number, price: number) => boolean;
    ...
    (set, get) => ({
      ...
      buy: (coin, usd, price) => {
        ...
        set({
          ...
          transactions: [{ id: uid(), type: "buy", coinId: coin.id, symbol: coin.symbol, amount, price, total: usd, status: "completed", createdAt: Date.now(), mode: "demo" }, ...s.transactions],
        });
        return true;
      },

      sell: (coinId, amount, price) => {
        const s = get();
        ...
        set({
          walletUSD: s.walletUSD + usd,
          holdings: newHoldings,
          transactions: [{ id: uid(), type: "sell", coinId, symbol: h.symbol, amount, price, total: usd, status: "completed", createdAt: Date.now(), mode: "demo" }, ...s.transactions],
        });
        return true;
      },
